...
async def play(client: Client, message_: Message):
    ...
    res = await message_.reply_text("GoodVibes=🔄 Işleme Alındı...")
    ...
    if message_.chat.id in tgcalls.pytgcalls.active_calls:
        ...
        await res.edit_text(f"〰️GoodVibes〰️=#️⃣ Konumda sıraya alındı {position}.")
    else:
        await res.edit_text("GoodVibes=▶️ Çalınıyor...")
        res.delete
        m = await client.send_photo(
        chat_id=message_.chat.id,
        photo="https://telegra.ph/GoodVibes-04-10",
        caption=f"Çalınıyor [GoodVibes](https://t.me/Fmsarkilar).",
         ) 
...
async def deezer(client: Client, message_: Message):
    ...
    if message_.chat.id in tgcalls.pytgcalls.active_calls:
        ...
        position = sira.add(message_.chat.id, file_path)
        await res.edit_text(f"GoodVibes=#️⃣ Konumda sıraya alındı {position}.")
    else:
        await res.edit_text("GoodVibes=▶️ Çalınıyor...")
        tgcalls.pytgcalls.join_group_call(message_.chat.id, file_path)
    await res.delete()
    m = await client.send_photo(
        chat_id=message_.chat.id,
        photo="final.png",
        caption=f"Çalınıyor [{title}]({url}) [Deezer](https://t.me/Fmsarkilar)."
    ) 
    os.remove("final.png")
# ---------------------------------------------------Jiosaavn-----------------------------------------------------------------
@Client.on_message(
...
async def jiosaavn(client: Client, message_: Message):
    ...
    m = await client.send_photo(
        chat_id=message_.chat.id,
        caption=f"Çalınıyor {sname} [Jiosaavn](https://t.me/Fmsarkilar)",
        photo="final.png",
    )
    os.remove("final.png")
...
async def ytp(client: Client, message_: Message):
    ...
    if message_.chat.id in tgcalls.pytgcalls.active_calls:
        ...
        await res.edit_text("GoodVibes=▶️Çalınıyor...")
        tgcalls.pytgcalls.join_group_call(message_.chat.id, file_path)
    await res.edit("Küçük Resim Oluşturuluyor.")
    ...
    m = await client.send_photo(
        chat_id=message_.chat.id,
        caption=f"Çalıyor `{query}` [YouTube](https://t.me/Fmsarkilar)",
        photo="final.png",
